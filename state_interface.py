...
class StateInterface(metaclass=abc.ABCMeta):
    ...
    def __init__(self, history_words, review_words_number, sessions_number):
        ...
        self._stu_excellent_df = pd.read_excel(STU_MEMORY_PATH, index_col=0, header=0)  # excellent students memory
        self._stu_forget_df = pd.read_excel(STU_MEMORY_PATH, index_col=0, header=0)  # forget students memory
    ...
    @property
    def stu_forget_df(self):
        """
                :return: the forget student memory.
                """
        return self._stu_forget_df

    @property
    def stu_excellent_df(self):
        """
                :return: the excellent student memory
                """
        return self._stu_excellent_df
